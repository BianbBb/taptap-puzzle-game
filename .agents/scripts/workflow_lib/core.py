...
def resolve_unity_cli(project):
    """Resolve a project-local Unity CLI, falling back to the user's PATH."""
    local = Path(project) / "Tools" / "unity.exe"
    if local.is_file():
        return local.resolve()
    global_cli = shutil.which("unity")
    return Path(global_cli).resolve() if global_cli else local


class Context:
    def __init__(self, project=None):
        self.project = Path(project).resolve() if project else (Path(__file__).resolve().parents[3] / "GameUnity").resolve()
        ...
        root_name = Path(__file__).resolve().parents[2].name
        self.scripts = self.repo / root_name / "scripts"
        self.runs = self.repo / ".agents" / "runs"
        self.cli = resolve_unity_cli(self.project)
        self.solution = self.project / "GameUnity.sln"
