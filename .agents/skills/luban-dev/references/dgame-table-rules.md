...
当前 DGame 业务表约定保持原始行序：普通表通常依次包含 `##var`、`##type`、`##group` 和 `##` 中文字段注释，但行号不是固定规则。新增字段必须复用原有注释行，不得把注释写入 `##group` 或另造无依据的注释行；追加前先删除字段区间内和末尾的空列。
...
数据从全部结构标记行之后开始。当前项目使用过的扩展类型包括 `vector2`、`vector3`、`vector4`、`vector2int`、`vector3int`，定义在 `GameConfig/Defines/builtin.xml`。
