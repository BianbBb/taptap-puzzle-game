...
class LubanConfigHelper:
    ...
    def add_table(self, full_name: str, fields: List[Dict], value_type: str = "",
                  ...
                  auto_import: bool = None, vertical: bool = False,
                  read_schema_from_file: Optional[bool] = None) -> bool:
        """新增配置表
        ...
        Args:
            ...
            read_schema_from_file: 是否从业务 Excel 的标记行读取 schema；不传时根据现有文件或新建表结构明确推断
        """
        ...
        data_file = self.data_dir / input_file
        if read_schema_from_file is None:
            read_schema_from_file = True
            if data_file.is_file():
                source_wb = None
                try:
                    source_wb = openpyxl.load_workbook(data_file, read_only=True, data_only=True)
                    source_sheet = (
                        source_wb[sheet_name]
                        if sheet_name and sheet_name in source_wb.sheetnames
                        else source_wb.active
                    )
                    read_schema_from_file = any(
                        source_sheet.cell(row=row, column=1).value in ("##var", "##type")
                        for row in range(1, min(source_sheet.max_row, 30) + 1)
                    )
                except Exception:
        ...
        if self.tables_file.exists():
            ...
            sheet.cell(row=row_num, column=4, value=bool(read_schema_from_file))  # read_schema_from_file
    ...
    def update_field(self, table_name: str, field_name: str,
        ...
        `new_type` 使用 `|` 为多行 `##type` 提供逐行类型。提供单个类型时，
        第一条 `##type` 写入该类型，剩余续行明确清空；提供的类型数量超过
        当前 `##type` 行数时拒绝修改，避免静默截断。
        ...
        try:
            ...
            type_rows = [row for row in range(1, sheet.max_row + 1)
                         if sheet.cell(row=row, column=1).value == "##type"]
            type_parts = None
            if new_type is not None:
                type_parts = [part.strip() for part in str(new_type).split("|")]
                if not type_rows or len(type_parts) > len(type_rows):
                    print(f"错误: 类型序列需要 {len(type_parts)} 行 ##type，但当前只有 {len(type_rows)} 行")
                    wb.close()
                    return False
            ...
            if new_type is not None:
                for offset, row in enumerate(type_rows):
                    # Explicit assignment to None is required to clear stale continuation
                    # cells in openpyxl; passing value=None to cell() does not clear them.
                    sheet.cell(row=row, column=field_col).value = (
                        type_parts[offset] if offset < len(type_parts) else None
                    )
    ...
    def import_json(self, table_name: str, input_file: str,
        ...
        if mode == "replace":
            raise ValueError(
                "replace 模式未实现；拒绝悄悄追加。请先确认明确的逐行修改范围。"
            )
...
def main():
    ...
    schema_group = table_add.add_mutually_exclusive_group()
    schema_group.add_argument("--read-schema-from-file", dest="read_schema_from_file", action="store_true",
                              help="明确从业务 Excel 的 ##var/##type 标记行读取 schema")
    schema_group.add_argument("--schema-in-defines", dest="read_schema_from_file", action="store_false",
                              help="明确使用 Defines/schema，不从业务 Excel 读取 schema")
    table_add.set_defaults(read_schema_from_file=None)
    ...
    field_update.add_argument("--type", default=None, help="新类型；多行复合类型按 ##type 行使用 | 分隔")
    ...
    import_parser.add_argument(
        "--mode", default="append", choices=["append", "replace"],
        help="导入模式；当前仅实现 append，replace 会明确拒绝"
    )
    ...
    # 表操作
        ...
        if args.table_command == "list":
            ...
            helper.add_table(
                ...
                vertical=vertical,
                read_schema_from_file=args.read_schema_from_file
            )
