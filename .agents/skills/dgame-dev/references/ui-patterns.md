...
public class ItemWidget : UIWidget
{
    ...
    {
        m_textName.text = cfg.ItemName;
        m_imgIcon.SetSprite(
            cfg.IconNameMan,
            cancellationToken: gameObject.GetCancellationTokenOnDestroy());  // 传入销毁令牌，避免窗口销毁后回调继续执行
    }
}
```
...
| Widget 资源放到 `Assets/Scripts` | DGame 热更 UI 在 `Assets/Scripts/HotFix/GameLogic/UI`，资源在 `Assets/BundleAssets` |
