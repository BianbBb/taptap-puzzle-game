...
- "Where should I create this skill? For this repository I will place it in `.agents/skills` so Claude Code and Codex share the same source; use `~/.agents/skills` only for a personal skill."
...
Before running `init_skill.py`, ask where the user wants the skill created. If the user is working in this repository and does not specify a location, default to `.agents/skills`; use `~/.agents/skills` for a personal skill.
